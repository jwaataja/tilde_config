module Tildeconfig
    ...
    class TildeMod
        ...
        ##
        # Set the root_dir for this module. All file source paths are relative to the value set
        # here. This is relative to the repository root.
        def root_dir(dir)
            if dir.nil?
                dir = '.'
            end
        ...
        ##
        # Set the install_dir for this module. All file dest paths are relative to the value set
        # here. `~` in the argument is automatically expanded to the user's home directory.
        def install_dir(dir)
            if dir.nil?
                dir = '~'
            end
            # expand relative to DEFAULT_INSTALL_DIR, not the current working directory
        ...
        def execute_install
            install_cmds.each { |action| action.call() }
        end

        ##
        # Execute the uninstall action for this module now.
        def execute_uninstall
            uninstall_cmds.each { |action| action.call() }
        end

        ##
        # Execute the update action for this module now.
        def execute_update
            update_cmds.each { |action| action.call() }
        end

        ##
        # Adds a file to be installed to this module. If only one argument is given, then it will
        # be used for both source and destination. Source is relative to repository root, or set
        # root_dir. Destination is relative to home directory, or dir set by install_dir.
      ...
      private
        ...
        def run_file_install(file_tuple)
            # TODO: this assumes that our working directory will always be the user's settings
            # repository.
        ...
        ##
        # Make a repeating prompt for Y/n answer, with an empty answer defaulting to no.
        # Returns true on yes, false on no or empty answer.
        def ask_yes_no(prompt)
            while true
                ...
                if res.start_with? /yY/
                    return true
                elsif res.start_with? /nN/ or res.strip.empty?
                    return false
                else
